...
public static class WindowsMaintenanceCommandCatalog
{
    ...
    [
        new(new MaintenanceCommandDefinition("nbtstat-purge-reload", CategoryCache, "NetBIOS 名前キャッシュを再読込", "nbtstat -R : キャッシュをパージしてローカル hosts から再読込する", false, false, true),
            "nbtstat", "-R"),
        new(new MaintenanceCommandDefinition("nbtstat-reregister", CategoryCache, "NetBIOS 名前を再登録", "nbtstat -RR : WINS サーバーへ名前を解放・再登録する", false, false),
            "nbtstat", "-RR"),
        new(new MaintenanceCommandDefinition("ipconfig-registerdns", CategoryCache, "DNS 登録を更新", "ipconfig /registerdns : この PC の DNS 名と IP アドレスを動的 DNS へ手動登録する", false, false),
        ...
        new(new MaintenanceCommandDefinition("arp-clear", CategoryCache, "ARP キャッシュをクリア", "netsh interface ipv4 delete arpcache : 全インターフェースの IPv4 ARP キャッシュを削除する", false, false, true),
            "netsh", "interface ipv4 delete arpcache"),
        new(new MaintenanceCommandDefinition("netsh-ipv4-delete-destinationcache", CategoryCache, "IPv4 経路キャッシュを削除", "netsh interface ipv4 delete destinationcache : 学習済みの次ホップ経路 (宛先キャッシュ) を削除する", false, false, true),
            "netsh", "interface ipv4 delete destinationcache"),
        new(new MaintenanceCommandDefinition("netsh-ipv6-delete-destinationcache", CategoryCache, "IPv6 経路キャッシュを削除", "netsh interface ipv6 delete destinationcache : 学習済みの次ホップ経路 (宛先キャッシュ) を削除する", false, false, true),
